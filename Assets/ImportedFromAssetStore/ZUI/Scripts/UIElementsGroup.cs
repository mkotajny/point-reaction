...
public class UIElementsGroup : ZUIElementBase
{
    ...
    void Start()
    {
        ...
        {
            if (!Prewarm)
                ChangeVisibility(Visible, IgnoreEventsOnInitialization);
            else
                ChangeVisibilityImmediate(Visible, IgnoreEventsOnInitialization);
        }
    }
    ...
    public float GetAllHidingTime()
    {
        ...
        for (int i = 0; i < AnimatedElements.Count; i++)
        {
            ...
            if (uiA.HideAfter + uiA.HidingDuration > hidingTime)
                hidingTime = uiA.HideAfter + uiA.HidingDuration;
